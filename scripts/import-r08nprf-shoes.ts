...
type Candidate = {
  ...
  realImageCountAfterVideoFilter: number;
  hasVideo: boolean;
  top: number;
  width: number;
  height: number;
...
const DEFAULT_URL = "https://shop01319897.wecatalog.cn/t/r08Nprf";
const SHELL_TEXT_RE = /A东成皮具|古奇\s*原厂皮|一间鞋厂|种草相册Lite|种草相册|微购相册|我的店铺|店铺已全新装修|就差你的光顾|欢迎光临|联系TA|全部上新|客服|购物车/g;
...
const VIDEO_URL_RE = /\.mp4(?:[?#]|$)|\.mov(?:[?#]|$)|\.m3u8(?:[?#]|$)|mediaType=video|type=video/i;
const VIDEO_FIELD_RE = /videoUrl|video_url|videoCover|video_cover|coverVideo|poster/i;
const STORE_TITLE_RE = /^(A东成皮具\s*【古奇\s*原厂皮】?|一间鞋厂|种草相册Lite.*)$/;
const BAD_TITLE_PART_RE = /A东成皮具|古奇\s*原厂皮|一间鞋厂|种草相册Lite|种草相册|微购相册|方便你也方便代理|我的店铺|店铺|店铺已全新装修|已全新装修|全新装修|就差你的光顾|光顾|欢迎光临|联系TA|全部上新|客服|购物车|鞋垫长度|^细节$/;
...
function parseArgs(): Options {
  ...
  return {
    ...
    outDir: read("--out-dir", path.join(process.cwd(), "imports", "wecatalog", `r08nprf-shoes-${timestamp}`)),
    debug: flag("--debug"),
    headed: flag("--headed"),
...
function isRejectedImageUrl(url: string) {
  return !url || BANNED_IMAGE_RE.test(url) || VIDEO_URL_RE.test(url) || /\.(svg|gif)(\?|$)/i.test(url);
}
...
function hasVideoValue(value: unknown): boolean {
  if (!value) return false;
  if (typeof value === "string") return VIDEO_URL_RE.test(value);
  if (Array.isArray(value)) return value.some((item) => hasVideoValue(item));
  if (typeof value === "object") {
    return Object.entries(value as Record<string, unknown>).some(([key, item]) => {
      if (/^(type|mediaType)$/i.test(key) && String(item || "").toLowerCase() === "video") return true;
      if (VIDEO_FIELD_RE.test(key)) {
        if (typeof item === "string") return item.trim().length > 0;
        if (Array.isArray(item)) return item.length > 0;
        return Boolean(item);
      }
      return hasVideoValue(item);
    });
  }
  return false;
}
...
function collectImageUrlsDeep(value: unknown, baseUrl: string, output: string[] = []) {
  ...
  if (typeof value === "string") {
    if (!VIDEO_URL_RE.test(value) && /^https?:\/\/|^\/\//.test(value) && /\.(jpe?g|png|webp)(?:[?#].*)?$/i.test(value)) {
...
function collectProductObjectsDeep(value: unknown, baseUrl: string, output: ApiItem[] = []) {
  ...
  const productLike = Boolean(
    item.goods_id || item.itemId || item.productId || item.id || item.shop_id || item.targetAlbumId ||
    item.imgsSrc || item.imgs || item.images || item.sourceList || item.miniapp_path || item.link
  );
  if (title && images.length >= 1 && productLike) {
    output.push(item);
  }
...
function imageUrlsFromApiItem(item: ApiItem, baseUrl: string) {
  const fromSourceList = Array.isArray(item.sourceList)
    ? item.sourceList.flatMap((source: ApiItem) => Array.isArray(source.medias) ? source.medias : [])
      .filter((media: ApiItem) => !hasVideoValue(media))
...
async function runScanOnly(options: Options) {
  ...
  let realImageCountGte9AfterVideoFilter = 0;
  ...
  for (const candidate of candidates) {
    ...
    if (candidate.realImageCountAfterVideoFilter >= options.minImages) {
      realImageCountGte9AfterVideoFilter += 1;
    }
  ...
  console.log(JSON.stringify({
    ...
    real_image_count_gte_9_after_video_filter: realImageCountGte9AfterVideoFilter,
...
async function fetchApiItems(options: Options) {
  const landing = await fetch(options.url, {
    redirect: "follow",
    headers: { "User-Agent": "Mozilla/5.0 LM-Dkbrand-local-importer r08Nprf" },
  });
  ...
  while (items.length < options.maxScan && isLoadMore && noNewPages < 3) {
    ...
    const response = await fetch(apiUrl, {
      ...
      headers: {
        ...
        "User-Agent": "Mozilla/5.0 LM-Dkbrand-local-importer r08Nprf",
        Referer: finalUrl,
        Cookie: cookie,
      },
    });
...
async function collectApiCandidates(options: Options) {
  ...
  for (const item of items) {
    const images = imageUrlsFromApiItem(item, finalUrl);
    const hasVideo = hasVideoValue(item);
    ...
    if (!rejectReason && hasVideo && images.length === 0) rejectReason = "video_candidate";
    ...
    candidates.push({
      ...
      realImageCountAfterVideoFilter: images.length,
      hasVideo,
  ...
  return raw
    ...
    .map((item, index) => {
      ...
      return {
        ...
        realImageCountAfterVideoFilter: imageUrls.length,
        hasVideo: isRejectedImageUrl(item.coverUrl),
...
async function exportProduct(detail: Detail, candidate: Candidate, options: Options, order: number, exported: ProductExport[]) {
  ...
  const productCode = `LM-SHO-R08NPRF-${String(order).padStart(4, "0")}`;
  ...
  const row: ProductExport = {
    ...
    import_batch_id: `r08nprf-shoes-${new Date().toISOString().slice(0, 10).replace(/-/g, "")}`,
    ...
    translation_status: "fallback",
    status: "draft",
    notes: "9 product images exported by r08Nprf-only importer",
  };
  exported.push(row);
...
async function runDebug(browser: Browser, options: Options) {
  ...
  for (const item of candidates.slice(0, 50)) {
    console.log(JSON.stringify({
      ...
      real_image_count_after_video_filter: item.realImageCountAfterVideoFilter,
...
async function runImport(browser: Browser, options: Options) {
  ...
  const writeFinal = async () => {
    ...
    await fs.writeFile(path.join(process.cwd(), ".last-wecatalog-shoes-products-file"), `${outputFile}\n`);
  };
  process.once("SIGINT", () => {
